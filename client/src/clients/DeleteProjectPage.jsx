...
const DeleteProjectPage = () => {
    ...
    const deleteProject = (ev, proj)=>{
      ev.stopPropagation();
      if (showDelModal && selectedProj?.id === proj.id) return; // Prevent re-trigger
    ...
    <div>
        ...
        <div className='project-details-in-page'>
            ...
            {userInfo && userInfo.projects && userInfo.projects.map((proj, idx)=>(
              <>
                <div className="my-project" onClick={ (ev)=>{deleteProject(ev, proj)}}>
                    {userInfo.role==='user'?
        ...
        {showDelModal && <DeleteProjectModal showModalSetter={setShowDelModal} proj={selectedProj}/>}
    </>
  )
}
